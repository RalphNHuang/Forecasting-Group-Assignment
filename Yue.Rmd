...
for (h in 1:28) {
  pred_list = lapply(hw_list, forecast.beta, h = h)
  hw_RMSSE_v = eval.RMSSE(testData, pred_list, h = h, devisor = devisors)
...
cat_devisors = unlist(lapply(trainAggCat[,2:ncol(trainAggCat)], cal.devisor))
cat_hw_RMSSE_v = eval.RMSSE(testAggCat, cat_pred_list, devisor = cat_devisors)
...
store_devisors = unlist(lapply(trainAggStore[,2:ncol(trainAggStore)], cal.devisor))
store_hw_RMSSE_v = eval.RMSSE(testAggStore, store_pred_list, devisor = store_devisors)
...
week_devisors = unlist(lapply(trainAggWeek[,2:ncol(trainAggWeek)], cal.devisor))
week_hw_RMSSE_v = eval.RMSSE(testAggWeek, week_pred_list, h = 4, devisor = week_devisors)
